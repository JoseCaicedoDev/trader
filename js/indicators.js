...
// Generic moving-average helpers operating on plain numeric arrays (not candle objects), so they
// can be applied both to price series and to derived series (e.g. running a MA over another MA's
// output, as the Oracle Move indicator does). Assume values contain only a leading run of nulls
// (the warm-up gap of whatever produced the series) with no gaps afterward — true for every series
// built in this codebase.
function firstValidIndex(values) {
...
function wmaSeries(values, period) {
  ...
  const start = firstValidIndex(values);
  if (start === -1 || period < 1) return out;
...
function emaSeries(values, period) {
  ...
  const out = new Array(n).fill(null);
  const start = firstValidIndex(values);
  if (start === -1 || period < 1 || start + period > n) return out;
  let sum = 0;
...
// Arnaud Legoux Moving Average: a Gaussian-weighted MA whose weight peak is offset within the
// window (offset=0.85 skews weight toward recent bars, sigma=6 controls the Gaussian's width),
// giving less lag than a plain WMA/SMA while staying smoother than an EMA. Standard formula.
function almaSeries(values, period, offset, sigma) {
  const n = values.length;
  const out = new Array(n).fill(null);
  const start = firstValidIndex(values);
  if (start === -1 || period < 1) return out;
...
function calculateWMA(data, period) {
  return wmaSeries(data.map(c => c.close), period);
}

function calculateALMA(data, period, offset = 0.85, sigma = 6) {
  return almaSeries(data.map(c => c.close), period, offset, sigma);
}
