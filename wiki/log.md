...
## [2026-07-25] sim | Estrategia 4 — ¿se pueden reducir las pérdidas? Stop de 5.5 a 5.0 ATR
- **Pregunta del usuario**, al ver en el historial pérdidas de −4.9% y −5.4% frente a ganancias de +1.9%: mejorar el stop para que las pérdidas sean menores.
- **Respuesta corta: casi nada funciona, porque la ventaja vive en el stop ancho.** Con el objetivo a 0.45R el punto de equilibrio es `1/(1+0.45)` = 69.0% de acierto, así que cualquier mecanismo que baje la tasa de acierto mata la estrategia — y todos los mecanismos de recorte de pérdidas la bajan. Medido sobre las 5 ventanas contra el 74.1% de acierto / +9.78% de referencia: breakeven armado a +0.20R → **56.7% / −0.53%**; a +0.30R → 65.0% / +0.86%; trailing 0.5R → **43.5% / +0.41%**; trailing 1.0R → 66.8% / +4.00%; stop temporal a 16 velas → 58.0% / +4.50%; a 36 velas → 66.7% / +7.90%. Ninguno supera a no hacer nada.
- **El diagnóstico que lo explica**: de las 57 salidas por stop, solo **33 llegaron a recorrer +0.20R a favor** antes de morir — las demás se fueron en contra casi desde la entrada y ningún breakeven las habría salvado. En cambio prácticamente **todas las ganadoras cotizaron por debajo de su entrada** en algún momento del camino al objetivo. El stop ancho es literalmente lo que compra el 74% de acierto.
- **La única variante que sobrevive**: ceñir el stop *manteniendo la distancia al objetivo*. `atrMult` 5.5 → **5.0** y `rrRatio` 0.45 → **0.495**, de modo que `atrMult * rrRatio` sigue siendo 2.475 ATR. Mismo objetivo, stop un 9% más cerca.
- **Lo que compra es riesgo, no retorno**: pérdida media −5.14% → **−4.73%**, peor pérdida −10.37% → **−9.43%**, drawdown medio 8.26% → **7.88%**, ventanas positivas 4/5 → **5/5** (peor ventana −0.1% → **+3.01%**), margen sobre el equilibrio 5.5 → **6.2 puntos**, ventana viva +8.58%/6.35% dd → **+9.02%/4.99% dd**. La esperanza por operación es **estadísticamente indistinguible** (+0.408% → +0.437%, t = 0.10): no debe leerse como una mejora de rentabilidad.
- **El argumento decisivo es la estabilidad**: 4.90–5.40 es una **meseta plana** donde todos los puntos mantienen 5/5, mientras que 5.50 estaba justo en su borde; la robustez del vecindario sube de **4% (1/25** puntos adyacentes que mantienen 5/5**) a 20% (5/25)**. En 35 ventanas solapadas de 1500 velas con paso 250 —para que el resultado no dependa de dónde se corta la historia— gana en **22 de 35**, con drawdown medio menor (7.11% vs 7.65%) y 31/35 positivas frente a 28/35.
- **Dos costes reales, registrados**: el camino combinado 2024-04..2026-07 baja de **+68.0% a +58.6%** (un solo camino, donde el interés compuesto amplifica diferencias pequeñas, frente a la media de ventanas y las deslizantes que ambas favorecen el cambio); y las 12 operaciones extra lo hacen algo más sensible a comisiones: al 0.30% por lado el ajuste anterior queda plano (+0.3%) y éste en −5.4%.
- **Estado tras el ajuste**: 5 ventanas 56.0 ops, 73.2% acierto, +10.65%, dd 7.88%, PF 1.33, **5/5**, peor ventana +3.01%. Combinada 301 ops, +58.65%, dd 11.51%, PF 1.23. Ventana viva 27 ops, 77.8%, +9.02%, dd 4.99%, PF 1.69. Racha perdedora máxima en 280 operaciones: 2.
- **Metodología**: el motor de investigación con gestión dinámica de stop (breakeven, trailing y stop temporal, todos en unidades de R y aplicados al cierre de cada vela para no mirar al futuro dentro de la vela) se verificó **idéntico a producción** con las opciones apagadas —ventana combinada, ventana viva, las 5 ventanas y la estrategia 2— antes de dar por buena ninguna medición. El resultado del candidato se reprodujo después con la propia `runEmaCrossStrategy` ya modificada, sin desviación.
- **Alcance**: solo `STRATEGY4_PARAMS` (una línea de `js/config.js`). Estrategias 2 y 3 (4h) intactas, según la restricción permanente del usuario.

---

## [2026-07-25] sim | Estrategia 4 — libro contrario tras take-profit (aplicado a petición expresa, no recomendado)
- **Idea del usuario**: cada vez que la estrategia cierra una operación en su objetivo, abrir en esa misma vela una posición contraria, independiente, que sale solo por su propio stop o take-profit y no encadena. Como máximo dos posiciones vivas a la vez: una de la estrategia y otra de la contraria (por ejemplo un corto venido de un TP conviviendo con un largo de la estrategia).
- **Se aplicó a petición expresa tras exponer las cifras.** La evidencia no lo respalda y así queda registrado tanto aquí como en el comentario de `STRATEGY4_PARAMS`.
- **La premisa se midió antes de implementar nada**: para los 143 cierres por TP de 2.3 años, el recorrido posterior a favor de la contraria alcanza su máximo en **+0.168% a 3 velas (p = 0.091)** contra un nulo por bootstrap de 20.000 remuestreos, se disuelve más allá de 5 velas y se vuelve negativo a 20. Con 8 horizontes probados el umbral de Bonferroni sería p < 0.006. Y el efecto es **menor que el coste de ida y vuelta (0.2%)**.
- **El libro contrario en solitario pierde dinero**: 2024-04 −5.23% · 2024-09 −23.86% · 2025-03 −1.83% · 2025-08 −10.28% · 2026-02 +22.88%. Una ventana de cinco positiva; combinada 103 operaciones, **−28.36%**, drawdown 44.52%, **PF 0.81**.
- **La aritmética que lo explica**: con `rrRatio` 0.45 el stop queda a 5.5 ATR y el objetivo a 2.475 ATR, así que el punto de equilibrio es `1/(1+0.45)` = **69.0% de acierto**, independiente de la volatilidad. La estrategia entrega 78.0% (margen +9.0); las entradas contrarias entregan 66.0% (**margen −2.9**). No pierden por mala suerte: pierden porque revertir tras un objetivo no tiene ventaja, así que aterrizan en el entorno del azar y la geometría hace el resto. 35 de 103 acabaron en stop, con pérdida media −5.35%.
- **Se barrieron 260 geometrías propias** de stop/objetivo para el libro contrario: las que dan 5/5 hacen entre 5 y 8 operaciones por ventana (una con PF 20.99). Muestras de ese tamaño entre 260 candidatas son ajuste de curva, no ventaja.
- **Por qué convence al mirar el gráfico**: la ventana viva de la app es justo la única que funciona — 10 operaciones, 80% de acierto, +8.17%, PF 1.84. Es el mismo sesgo de ventana reciente que ya apareció al analizar la asimetría largos/cortos.
- **Resultado aplicado, cuenta completa (50% estrategia + 50% contraria, que es lo que significan dos posiciones simultáneas sin apalancamiento)**: 54.0 operaciones/ventana = **2.27 por semana** (antes 1.45, un 57% más — lo más cerca que se ha estado del objetivo de actividad), acierto 74.1%, retorno **+9.78%/ventana** (antes 23.23%), drawdown medio **8.26%** (antes 14.83%, mejora por diversificación), peor ventana **−0.05%** y por tanto **4/5** (antes 5/5), PF 1.31 (antes 1.62). Combinada: 289 operaciones, **+67.96%**, drawdown 13.56%, PF 1.26 — frente a +164.28% de la estrategia sola al 100%. Referencia clave: la estrategia al 50% con la otra mitad en efectivo daría +82.1%, así que **el libro contrario cuesta unos 14 puntos** respecto a no hacer nada con ese capital.
- **Implementación**: `counterOnTP` (más `counterAtrMult`, `counterRrRatio` y `counterOnlyAfter`, opcionales) en `js/strategy-emacross.js`. Deliberadamente **no** se usa `runSimulator` para el libro contrario: éste debe **saltarse** un disparo que llegue con posición ya abierta, mientras que `runSimulator` lo trataría como reversión; el resto (stop antes que objetivo dentro de la vela, comisiones, dimensionamiento) replica sus reglas. `mergeBooks` produce la vista de cuenta completa (curva de capital, balance, drawdown y profit factor del conjunto) y deja los dos libros accesibles en `backtest.books`.
- **UI**: las operaciones del libro contrario se etiquetan con `eventCode: 'COUNTER_ON_TP'` ("Contraria tras TP" en `EVENT_LABELS`). `trades-table.js` y `chart-manager.js` pasan a preferir la etiqueta propia de la operación sobre el array por vela, necesario porque el libro contrario comparte índices de vela con las salidas de la estrategia y el array no puede distinguirlos; sin `eventCode` el comportamiento es el de siempre.
- **Tres correcciones en el historial de operaciones**, detectadas al revisar la tabla contra el gráfico:
  1. **Emparejamiento por libro**. `TradesTable.render` recorría las operaciones asumiendo una sola posición abierta, así que con dos libros emparejaba la entrada de un libro con la salida del otro: aparecían largos "ganadores" saliendo por debajo de su precio de entrada, y entradas y salidas con la misma marca de tiempo. Ahora se empareja dentro de cada libro y las filas se ordenan por fecha de salida.
  2. **Orden estable dentro de la misma vela**. El comparador de `mergeBooks` devolvía −1 para cualquier par cuyo primer elemento fuera de la estrategia, incluso comparando dos operaciones del mismo libro, rompiendo el orden original cuando una salida por stop y la reversión que abre a continuación comparten vela. Corregido para ordenar solo *entre* libros; con esto la tabla pasó de 23 filas con una incoherente a 24 filas, todas coherentes, cuadrando con las 24 operaciones del motor.
  3. **Saldo de cuenta**. La columna mostraba `equity`, que es el saldo del libro concreto sobre base 100 y saltaba de forma errática al intercalar los dos libros. `mergeBooks` añade ahora `accountEquity` (saldo de la cuenta completa en esa vela) y la tabla lo prefiere. La última fila coincide con `finalBalance`.
  Además, las entradas llevan ahora su propia etiqueta (`eventCode` tomado de `entryLabels`): antes, cuando un stop y su reversión caían en la misma vela, la entrada se mostraba como "Stop Loss" porque `runSimulator` sobrescribe esa posición del array con el motivo de salida. Es un defecto que existía desde antes del libro contrario y afectaba también a las estrategias 2 y 3. Verificado que la corrección es solo de presentación: los resultados numéricos (balance final, drawdown, profit factor y todos los campos de cada operación) siguen idénticos.
- **No regresión verificada**: estrategias 2 y 3 idénticas byte a byte, y el motor base sin re-entrada, sin filtro y sin contraria también idéntico byte a byte al código original. `counterOnTP: false` desactiva todo esto.
